#!/bin/bash
# ==============================================================================
# Claude Code <-> ds4-server 会话数据与配置双向同步工具
# ==============================================================================
# 用途：
#   解决官方 Claude Code (~/.claude) 与本地 ds4-server (~/.ds4/claude_config)
#   因目录隔离导致的 --resume 历史会话无法读取、全局规则/Skills 不同步的问题。
#
# 使用方式：
#   ./sync_sessions.sh              # 默认：将官方 ~/.claude 会话增量同步到 ds4
#   ./sync_sessions.sh -b           # 双向同步 (两边均保留最新文件)
#   ./sync_sessions.sh -r           # 反向同步：将 ds4 会话同步回官方 ~/.claude
...
DS4_DIR="$HOME/.ds4/claude_config"
...
选项:
  -f, --from-official   从官方 Claude (~/.claude) 同步到 ds4 配置目录 (默认行为)
  -r, --to-official     从 ds4 配置目录 (~/.ds4/claude_config) 反向同步到官方 Claude
  -b, --two-way         双向增量同步 (两边各自同步更新的文件)
  -l, --link-shared     仅软链接全局配置文件 (CLAUDE.md, writing-style.md, skills)
  -h, --help            显示帮助信息

示例:
  $(basename "$0")                                     # 默认增量同步所有官方会话到 ds4
  $(basename "$0") -b                                  # 双向同步
...
link_shared_configs() {
    ...
    for file_name in "${SHARED_FILES[@]}"; do
        if [ -f "$OFFICIAL_DIR/$file_name" ]; then
            if [ ! -e "$DS4_DIR/$file_name" ]; then
                ln -sf "$OFFICIAL_DIR/$file_name" "$DS4_DIR/$file_name"
                printf "  ${GREEN}✓${NC} 链接文件: %s -> ~/.ds4/claude_config/%s\n" "$file_name" "$file_name"
            fi
        fi
    done
    ...
    for dir_name in "${SHARED_DIRS[@]}"; do
        if [ -d "$OFFICIAL_DIR/$dir_name" ]; then
            if [ ! -e "$DS4_DIR/$dir_name" ]; then
                ln -sf "$OFFICIAL_DIR/$dir_name" "$DS4_DIR/$dir_name"
                printf "  ${GREEN}✓${NC} 链接目录: %s -> ~/.ds4/claude_config/%s\n" "$dir_name" "$dir_name"
            fi
        fi
    done
}
...
sync_specific_session() {
    ...
    while IFS= read -r project_path; do
        if [ -n "$project_path" ]; then
            ...
            local ds4_proj_dir="$DS4_DIR/projects/$rel_project"
            mkdir -p "$ds4_proj_dir"

            # 复制 jsonl 与可能存在的目录
            if [ -f "$project_path/$target_uuid.jsonl" ]; then
                cp -f "$project_path/$target_uuid.jsonl" "$ds4_proj_dir/"
                found_count=$((found_count + 1))
            fi
            if [ -d "$project_path/$target_uuid" ]; then
                cp -rf "$project_path/$target_uuid" "$ds4_proj_dir/"
                found_count=$((found_count + 1))
            fi
        fi
    ...
    for sub in "session-env" "file-history" "tasks"; do
        if [ -e "$OFFICIAL_DIR/$sub/$target_uuid" ]; then
            mkdir -p "$DS4_DIR/$sub"
            cp -rf "$OFFICIAL_DIR/$sub/$target_uuid"* "$DS4_DIR/$sub/" 2>/dev/null || true
            found_count=$((found_count + 1))
        fi
    done

    if [ "$found_count" -gt 0 ]; then
        printf "${GREEN}✓ 成功同步会话 [%s] 至 ds4 配置环境！${NC}\n" "$target_uuid"
    else
...
case "$SYNC_MODE" in
    "single-session")
        ...
        ;;
    "from-official")
        sync_directory_data "$OFFICIAL_DIR" "$DS4_DIR" "~/.claude -> ~/.ds4/claude_config"
        ;;
    "to-official")
        sync_directory_data "$DS4_DIR" "$OFFICIAL_DIR" "~/.ds4/claude_config -> ~/.claude"
        ;;
    "two-way")
        sync_directory_data "$OFFICIAL_DIR" "$DS4_DIR" "1. ~/.claude -> ~/.ds4/claude_config"
        sync_directory_data "$DS4_DIR" "$OFFICIAL_DIR" "2. ~/.ds4/claude_config -> ~/.claude"
        ;;
    "link-only")
